...
Session.Session = function() {
    ...
    this.ambush_tags_by_obj_id = {}; // GUI purposes only - identify unique factories (same as minefield tags above)
...
Session.Session.prototype.on_real_world_object_added = function(event) {
    ...
    if(obj.is_building() && obj.team == 'player') {
        if(obj.is_minefield()) {
            ...
            var count = goog.object.getCount(this.ambush_tags_by_obj_id[obj.spec['name']] || {});
            var tag = String.fromCharCode('A'.charCodeAt(0) + count).toString();
            if(!(obj.spec['name'] in this.ambush_tags_by_obj_id)) {
                this.ambush_tags_by_obj_id[obj.spec['name']] = {};
                this.ambush_tags_by_tag[obj.spec['name']] = {};
            }
            this.ambush_tags_by_obj_id[obj.spec['name']][obj.id] = tag;
            this.ambush_tags_by_tag[obj.spec['name']][tag] = obj.id;
