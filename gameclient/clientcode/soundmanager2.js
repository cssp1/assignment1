...
// SP3RDPARTY : soundmanager2.js : BSD License
...
function SoundManager(smURL, smID) {
   ...
   *
   * @param {?} oOptions Sound options (at minimum, id and url are required.)
   ...
   * @param {?} oMethod The relevant callback to fire
   * @param {?} oScope Optional: The scope to apply the callback to
   ...
   * @param {string} sID The ID of the sound
   * @param {number} nPosition The position to watch for
   * @param {?} oMethod Optional: The relevant callback to fire
   * @return {SMSound} The SMSound object
   */
   ...
   *
   * @param {?} oLink an HTML DOM &lt;a&gt; object or object literal including href and/or type attributes
   * @return {boolean} URL playability
   */
   ...
   *
   * @return {?number} Percent loaded, or if invalid/unsupported, null.
   */
  ...
  _normalizeMovieURL = function(smURL) {
    ...
    var fields = smURL.split('?');
    smURL = fields[0];
    var extra_params = (fields.length > 1 ? fields[1] : null);
...
//    }
  ...
  _debugTS = function(sEventType, bSuccess, sMessage) {
    ...
    if (typeof window['sm2Debugger'] !== 'undefined') {
      try {
        window['sm2Debugger'].handleEvent(sEventType, bSuccess, sMessage);
      } catch(e) {
        // oh well
      }
    }
  ...
  _detectFlash = function() {
    ...
    var hasPlugin = false, n = navigator, nP = n.plugins, obj, type, types, AX = _win['ActiveXObject']; // DJM for closure
  ...
  _parseURL = function(url) {
     ...
     * @param {(string|array)} url A single URL string, OR, an array of URL strings or {url:'/path/to/resource', type:'audio/mp3'} objects.
     */
...
if (typeof window['SM2_DEFER'] === 'undefined' || !window['SM2_DEFER']) {
    console.log("SM2_DEFER IS FALSE");
    console.log(window);
