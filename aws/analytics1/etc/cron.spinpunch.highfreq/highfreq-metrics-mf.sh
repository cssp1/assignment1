...
(cd $GAME_DIR/gameserver && nice ./dump_userdb.py --cache-only \
    ...
    --cache-read logs/marsfrontier-upcache --from-s3-bucket spinpunch-upcache --from-s3-keyfile $AWSSECRET \
    --cache-write logs/marsfrontier-upcache --to-s3-bucket spinpunch-upcache --to-s3-keyfile $AWSSECRET) > /dev/null
