resource "aws_alb" "game_alb" {
  ...
  security_groups = [data.terraform_remote_state.corp.outputs.spinpunch_prod_fe_elb_security_group_id]
  subnets = split(
    ",",
    data.terraform_remote_state.corp.outputs.spinpunch_prod_subnet_ids,
  )
  idle_timeout                     = 600
...
resource "aws_alb_listener" "game_https" {
  ...
  certificate_arn   = data.terraform_remote_state.corp.outputs.spinpunch_ssl_certificate_id
  default_action {
    target_group_arn = aws_alb_target_group.game_haproxy.arn
    type             = "forward"
  }
