...
module "game_server_mf" {
  ...
  sitedomain = "${var.sitedomain}"
  region = "${var.region}"
  ami = "${module.aws_cloud_init_mf.current_amazon_linux_ami_id}"
  key_pair_name = "${data.external.management_secrets.result["SSH_KEYPAIR_NAME"]}"
  aws_cloud_config_head = "${module.aws_cloud_init_mf.cloud_config_head}"
  aws_cloud_config_tail = "${module.aws_cloud_init_mf.cloud_config_tail}"
  aws_ec2_iam_role_fragment = "${module.aws_cloud_init_mf.ec2_iam_role_fragment}"
  cron_mail_sns_topic = "${data.terraform_remote_state.corp.cron_mail_sns_topic}"
...
data.terraform_remote_state.game_frontend.cloudfront_ingress_security_group_id_list)}"
  ...
  # to take over as the master, set snam to "srv0" (or blank?) and make DNS entries "mfprod-raw.spinpunch.com" and "mfprod-srv0.spinpunch.com"
}
