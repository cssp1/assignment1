...
variable "backups_bucket" {
  default = "spinpunch-mongolith-backups"
}

variable "extra_backups_bucket" {
  default = "spinpunch-backups"
}
...
module "mongodb" {
  ...
  cron_mail_sns_topic          = data.terraform_remote_state.corp.outputs.cron_mail_sns_topic
  ssh_access_security_group_id = data.terraform_remote_state.corp.outputs.spinpunch_ssh_access_security_group_id
  mongodb_instance_type        = var.mongodb_instance_type
  n_instances                  = var.n_instances
  mongodb_security_group_id    = data.terraform_remote_state.corp.outputs.spinpunch_prod_mongodb_security_group_id
}
