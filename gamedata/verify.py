...
def check_mandatory_fields(specname, spec, kind):
    ...
    if kind == 'units':
        ...
        elif gamedata['game_id'] == 'sg' and (spec.get('resurrectable',False) or (not spec.get('consumable',False))):
            error |= 1; print '%s: units in SG must be "resurrectable":0, "consumable":1 (except for hero units - when implemented, except them from this error).' % specname
...
PREDICATE_TYPES = set(['AND', 'OR', 'NOT', 'ALWAYS_TRUE', 'ALWAYS_FALSE', 'TUTORIAL_COMPLETE', 'ACCOUNT_CREATION_TIME',
                   ...
                   'RESOURCES_HARVESTED_TOTAL', 'RESOURCES_HARVESTED_AT_ONCE', 'FRIENDS_JOINED', 'FACEBOOK_LIKES_SERVER',
...
CONSEQUENT_TYPES = set(['NULL', 'AND', 'RANDOM', 'IF', 'COND', 'LIBRARY',
                        ...
                        'INVOKE_CRAFTING_DIALOG', 'INVOKE_MISSIONS_DIALOG', 'INVOKE_STORE_DIALOG', 'INVOKE_BUY_GAMEBUCKS_DIALOG',
                        ...
                        'ENABLE_COMBAT_RESOURCE_BARS', 'ENABLE_DIALOG_COMPLETION', 'TAKE_ITEMS',
...
def check_consequent(cons, reason = '', context = None, context_data = None):
    ...
    if cons['consequent'] == 'IF':
        ...
        if not cons.get('name', '') in gamedata['daily_tips']:
...
def check_quests(quests):
    ...
    for key, data in quests.iteritems():
        ...
        for CONS in ('ui_accept_consequent',):
