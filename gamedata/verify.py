...
def check_power_settings(specname, spec):
    error = 0
    if 'power_shutdown_threshold' in spec:
        if isinstance(spec['power_shutdown_threshold'], list):
            last_threshold = 1.0
            for i, threshold in enumerate(spec['power_shutdown_threshold']):
                if not isinstance(threshold, float):
                    error |= 1; print '%s power_shutdown_threshold list entries should all be float values' % (specname)
                if threshold > last_threshold:
                    error |= 1; print '%s power_shutdown_threshold value grows from L%d to L%d. They should reduce or stay the same' % (specname, i, i + 1)
                last_threshold = threshold
        else:
            if not isinstance(spec['power_shutdown_threshold'], float):
                error |= 1; print '%s power_shutdown_threshold should be a float value or a list' % (specname)
            if spec['power_shutdown_threshold'] >= 1.0:
                error |= 1; print '%s power_shutdown_threshold should be less than 1.0, value is %d' % (specname, spec['power_shutdown_threshold'])
    if 'damaged_power_output' in spec:
        if isinstance(spec['damaged_power_output'], list):
            last_output = 0.01
            for i, output in enumerate(spec['damaged_power_output']):
                if not isinstance(threshold, float):
                    error |= 1; print '%s damaged_power_output list entries should all be float values' % (specname)
                if output < last_threshold:
                    error |= 1; print '%s damaged_power_output value shrinks from L%d to L%d. They should increase or stay the same' % (specname, i, i + 1)
                last_output = output
        else:
            if not isinstance(spec['damaged_power_output'], float):
                error |= 1; print '%s damaged_power_output should be a float value or a list' % (specname)
            if spec['damaged_power_output'] <= 0.0:
                error |= 1; print '%s damaged_power_output should be greater than 0.0, value is %d' % (specname, spec['damaged_power_output'])
    return error
...
#                error |= 1; print 'building %s has invalid exclusion_zone setting. In SG, all buildings (except barriers) must have "exclusion_zone": [4,4]' % specname
    ...
    if 'proportionate_power_threshold' in spec and 'provides_power' not in spec:
        error |= 1; print '%s has proportionate_power_threshold but not provides_power' % (specname)
...
def check_aura(auraname, spec, maxlevel):
    ...
    if 'code' in spec:
        ...
        for effect in spec['effects']:
            ...
            # client-side codes (implemented in main.js)
            elif effect['code'] in ('speed_boosted', 'speed_weakened', 'defense_boosted', 'defense_weakened', 'radiation_hardened', 'grounded', 'make_invuln', 'make_invuln_invisible', 'is_invuln', 'ai_missile_defending',
                                    'frozen', 'ice_shielded', 'ice_encrusted', 'moving_in_swamp', 'swamp_shielded', 'range_booster_invisible', 'damage_weakened',
                                    ...
                                    'on_fire', 'projectile_speed_reduced', 'range_booster', 'sprite_swapped', 'cast_spell_continuously', 'avoided_detection',
...
simple_level_re = re.compile(r'(?P<root>.+)_L(?P<level>[0-9]+)$')
...
PREDICATE_TYPES = set(['AND', 'OR', 'NOT', 'ALWAYS_TRUE', 'ALWAYS_FALSE', 'TUTORIAL_COMPLETE', 'ACCOUNT_CREATION_TIME', 'HAS_ALTS', 'ALLIANCE_ID',
                   ...
                   'HOSTILE_UNIT_NEAR', 'HOSTILE_UNIT_EXISTS', 'DAY_OF_WEEK', 'HAS_ACHIEVEMENT', 'NO_REGION',
...
#                print '%s: EVENT_TIME predicate uses incorrect kind "%s" - it should be pvp not pve' % (reason, pred['event_kind'])
    ...
    elif pred['predicate'] == 'ENHANCEMENT_LEVEL':
        if pred.get('enhancement',None) not in gamedata['enhancements']:
            error |= 1
            print '%s: %s predicate refers to nonexistent enhancement "%s"' % (reason, pred['predicate'], pred.get('enhancement','MISSING'))
    ...
    elif pred['predicate'] == 'OBJECT_OWNERSHIP':
...
CONSEQUENT_TYPES = set(['NULL', 'AND', 'RANDOM', 'IF', 'COND', 'LIBRARY',
                        'PLAYER_HISTORY', 'GIVE_LOOT', 'SESSION_LOOT', 'GIVE_TROPHIES', 'GIVE_TECH', 'GIVE_ENHANCEMENT', 'APPLY_AURA', 'REMOVE_AURA', 'REMOVE_PROTECTION', 'COOLDOWN_TRIGGER', 'COOLDOWN_TRIGGER', 'COOLDOWN_RESET',
...
def check_consequent(cons, reason = '', context = None, context_data = None):
    ...
    if cons['consequent'] == 'IF':
        ...
        if (cons.get('reason',None) != context and cons.get('reason',None) not in ('special', 'promo_code', 'login_incentive')) or ('reason' not in cons and 'context' not in cons):
            error |= 1
            print '%s: GIVE_LOOT consequent has bad "reason", it should match the "context" (currently %s) or be either "special", "promo_code", or "login_incentive". It cannot be blank unless "context" is set!' % (reason, str(context))
            ...
            if FIELD not in cons:
                error |= 1; print '%s: %s consequent missing mandatory field %s' % \
                         (reason, cons['consequent'], FIELD)
                                ...
                                'HELP_REQUEST_REMINDER', 'REMOVE_PROTECTION',
                                'CLEAR_UI', 'CLEAR_NOTIFICATIONS', 'DEV_EDIT_MODE', 'GIVE_GAMEBUCKS', 'LOAD_AI_BASE', 'REPAIR_ALL', 'FPS_COUNTER',
                                'FOCUS_CHAT_GUI', 'ALL_AGGRESSIVE', 'INVITE_COMPLETE', 'INVOKE_LOGIN_INCENTIVE_DIALOG', 'INVOKE_PRIVACY_DIALOG', 'INVOKE_FULLSCREEN_PROMPT',
                                'NULL']:
        ...
        if '_blueprint' in sku['item']:
            ...
            if match: # per-level blueprint
                ...
                for level in xrange(1, my_level+1):
                    ...
                    expect_library_preds.add(root + ('_L%d_available' % (level)))
                ...
                expect_library_preds.add(sku['item'][:-len('_blueprint')]+'_multiple_blueprints') # use this to hide single blueprint if multiple blueprints are released later
...
def main(args):
    ...
    if 'payments_api' in gamedata['store'] and type(gamedata['store']['payments_api']) is list and gamedata['game_id'] in ('tr','eg'):
        for platform in ('fb','k2','ag','bh'):
            ...
            for predicate in gamedata['store']['payments_api']:
                ...
                if predicate == [{"predicate": "LIBRARY", "name": "electron_use_xsolla"}, "xsolla"]: continue
    ...
    for name, data in gamedata['tints'].iteritems():
        ...
        #print(template)
        for state, statedata in data.iteritems():
