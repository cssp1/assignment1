...
# SP3RDPARTY : SortedCollection.py : MIT License
# SP3RDPARTY : blist Python module : BSD License
...
try:
    ...
    print >> sys.stderr, "SortedCollection: Python blist module not found, performance degraded"
    pass
...
class SortedCollection(object):
    ...
    def __init__(self, iterable=(), key=None):
        ...
        if has_blist:
            list_klass = blist.blist
        else:
            list_klass = list

        if key is None: # items are their own keys
            key = (lambda x: x)
            self._keys = list_klass(sorted(iterable))
            self._items = list_klass(self._keys) # duplicate, do not share this reference
        else:
            decorated = sorted((key(item), item) for item in iterable)
            self._keys = list_klass(k for k, item in decorated)
            self._items = list_klass(item for k, item in decorated)
