    ...
    def punish_player(self, user_id, cur_region_name, last_login_ip, ip_rep_result):
        ...
        if not is_repeat_offender:
            if is_majority_anti_vpn_game:
                message_body = 'We identified your connection method as a VPN and therefore relocated your base to %s. If we falsely identified your connection as a VPN, please contact support and our team will be able to assist you with the case. However, note that a repeated violation of our anti-VPN policy will result in a permanent ban from map regions.' % (new_region['ui_name'])
            else:
                message_body = 'We identified your connection method as a VPN while using anti-VPN region %s and therefore relocated your base to %s. If we falsely identified your connection as a VPN, please contact support and our team will be able to assist you with the case. However, note that a repeated violation of our anti-VPN policy will result in a permanent ban from anti-VPN maps.' % (cur_region['ui_name'], new_region['ui_name'])
...
class AltPolicy(Policy):
    ...
    def punish_player(self, user_id, master_id, cur_region_name, other_alt_region_names, all_alt_ids, alt_limit = 0):
        ...
        if len(anti_alt_regions) < 1 or len(pro_alt_regions) < 1:
            anti_alt_region_names = []
            pro_alt_region_names = []
            for x in anti_alt_regions.itervalues():
                anti_alt_region_names.append(x['id'])
            for x in pro_alt_regions.itervalues():
                pro_alt_region_names.append(x['id'])
            sys.stderr.write('error punishing %s user %d in region %s: anti_alt_regions length is %d (values %r), pro_alt_regions length is %d (values %r)' % (SpinConfig.game(), user_id, cur_region_name, len(anti_alt_regions), str(anti_alt_region_names), len(pro_alt_regions), str(pro_alt_region_names)))
