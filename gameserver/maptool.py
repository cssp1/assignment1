...
def nosql_read_all_objects(region_id, base_id, base_landlord_id):
    ...
    for state in nosql_client.get_mobile_objects_by_base(region_id, base_id) + nosql_client.get_fixed_objects_by_base(region_id, base_id):
        ...
        for FIELD in ('level', 'hp_ratio', 'tag', 'metadata', 'creation_time', 'repair_finish_time', 'disarmed',
                      'upgrade_total_time', 'upgrade_start_time', 'upgrade_done_time', 'upgrade_ingredients', 'upgrade_help', 'upgrade_helped', 'squad_id',
...
def abandon_quarry(db, lock_manager, region_id, base_id, days_to_claim_units = 3, feature = None, dry_run = True):
    ...
    if not dry_run:
        ...
        unset_props = ['upgrade_total_time','upgrade_start_time','upgrade_done_time','upgrade_ingredients','upgrade_help','upgrade_helped',
                       'enhancing','crafting']
    ...
    for spawn_data in spawn_list:
        ...
        for i in xrange(qty):
            if spawn_hive(hives, map_cache, db, lock_manager, region_id, (spawn_data['id_start']+i), name_idx, spawn_data['template'], template,
                          ...
                          duration = int(spawn_data.get('duration',hives['duration']) *
                                         (1.0 - spawn_data.get('randomize_duration',hives['randomize_duration']) * random.random())),
                          region_player_pop = player_pop if region_data.get('hive_num_scale_by_player_pop',False) else None,
                          force_rotation = force_rotation, dry_run = dry_run):
...
def spawn_hive(hives, map_cache, db, lock_manager, region_id, id_num, name_idx, template_name, template,
               start_time = -1, end_time = -1, repeat_interval = None, duration = -1,
               force_rotation = False, region_player_pop = None,
               dry_run = True):
...
def resolve_raid_squads(db, lock_manager, region_id, dry_run = True):
    ...
    raid_squads = filter(lambda squad: ('base_map_path' not in squad) or (squad['base_map_path'][-1]['eta'] < time_now + gamedata['server'].get('map_path_fudge_time',0.25) + 1.0), raid_squads)
    ...
    for squad in raid_squads:
        ...
        # (see Region.py for explanation of the fudge_time and 1.0 here)
        if 'base_map_path' in squad and squad['base_map_path'][-1]['eta'] >= time_now - gamedata['server'].get('map_path_fudge_time',0.25) - 1.0:
