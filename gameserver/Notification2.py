...
STREAM_URGENT = 'URGENT' # got attacked etc
...
def ref_to_stream(ref):
    return {None: STREAM_URGENT,
            ...
            'you_sent_gift_order': STREAM_URGENT,
            'you_got_gift_order': STREAM_URGENT,
            'your_gift_order_was_received': STREAM_URGENT,
            'alliance_promoted': STREAM_URGENT,
            'alliance_demoted': STREAM_URGENT,
            'bh_invite_accepted_sender': STREAM_URGENT,
            'bh_invite_accepted_target': STREAM_URGENT,
            'bh_invite_completed_sender': STREAM_URGENT,
            'bh_invite_completed_target': STREAM_URGENT,
...
def can_send(time_now, timezone, stream, ref, history, cooldowns, user_class):
    ...
    if user_class is USER_NEW and stream is not STREAM_RETENTION:
        return False, 'cannot send notification streams other than retention to TH<2 users'
...
#        return False, 'cannot send progression stream until TH 3+'
    ...
    if stream is not STREAM_URGENT:
    ...
    # non-retention messages
        ...
        # don't send more than once per stream since logout
        ...
        if ref:
            max_unacked = {STREAM_URGENT: 5}.get(stream, 3)
