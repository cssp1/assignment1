...
def item_is_giveable(gamedata, spec):
    ...
    if spec['name'] in ('instant_repair', 'token', 'friendstone'): return True
...
def do_action(path, method, args, spin_token_data, nosql_client):
    try:
        ...
        if path[0] == 'player':
            ...
            # require special role for writes, except for gag/ungag
            if (method not in ('lookup','get_raw_player','chat_gag','chat_ungag')):
...
def get_server_latency(nosql_client):
    ...
    rows = list(nosql_client.server_latency_table().find({'$or':[{'time':{'$gte':time_range[0],'$lt':time_range[1]},
                                                          ...
                                                          }, {'time':1,'latency':1,'ident':1}).sort([('time',1)]))
