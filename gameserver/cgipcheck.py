...
def identify_accounts_to_reban():
    ...
    for user_id in accounts[0:3]:
        ...
        for entry in player['history']['customer_support']:
            if entry['method'].upper() == 'BAN':
                ban_count += 1
            elif entry['method'].upper() == 'UNBAN':
                unban_count += 1
        if ban_count > unban_count:
            reban_accounts(str(user_id))
    return 'Checked ' + str(accounts[0:3]) + ', need to ban ' + str(reban_accounts)
...
def do_gui(spin_token_data, spin_token_raw, spin_token_cookie_name, spin_login_hint_cookie_name, my_endpoint, nosql_client):
    ...
    replacements = {
        ...
        '$VISIT_BASE_URL$': 'https://apps.facebook.com/%s/?visit_base=' % SpinConfig.config['facebook_app_namespace'] if SpinConfig.config.get('facebook_app_namespace') else '',
