...
# dt: the time offset between successive intervals, in seconds
...
def update_summary(sql_util, con, cur, table, affected, source_range, interval, dt,
                   origin = 0, verbose = True, dry_run = False, execute_func = None,
                   resummarize_tail = 0):
    ...
    if rows and rows[0] and rows[0]['begin'] and rows[0]['end']:
        ...
        if source_range: # fill in any missing trailing summary data
            source_days = sorted(affected.union(set(xrange(dt*((rows[0]['end']-origin + dt - resummarize_tail)//dt)+origin,
                                                           dt*((source_range[1]-origin)//dt + 1)+origin, dt))))
        else:
            source_days = sorted(list(affected))
    else:
        # recreate entire summary
        if source_range:
            source_days = range(dt*((source_range[0]-origin)//dt)+origin, dt*((source_range[1]-origin)//dt + 1)+origin, dt)
        else:
            source_days = None

    if source_days:
        for day_start in source_days:
            if verbose: print 'updating', table, 'at', day_start, time.strftime('%Y%m%d', time.gmtime(day_start))
