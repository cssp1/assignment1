...
TUTORIAL_AI = {
    ...
    'dv': [{'ui_name': 'Red Pole', 'key': 'ai_redpole_progress', 'level_to_key': [0,3,4,5,6,7,8,9,10]},
           {'ui_name': 'Mr. Skilling', 'key': 'ai_mrskilling_progress', 'num_levels':25},
           ],
...
#    {'name': 'F07 Used Feature: Settings Menu', 'func': lambda user: user.get('feature_used:settings_dialog',False) },
    ...
    {'name': 'Q22B Destroyed 2+ Hives', 'func': lambda user: user.get('hives_destroyed',0) >= 2 },
    {'name': 'Q22C Destroyed 3+ Hives', 'func': lambda user: user.get('hives_destroyed',0) >= 3 },
...
#    {'name': 'X02 Payer Promo Conversion Rate', 'mandatory_func': lambda user: user.get('payer_promo_offered',0) > 0, 'func': lambda user: user.get('promo_gamebucks_earned',0) > 0 },
    ...
    {'name': 'Z01A %CONV% Made 3 Payments', 'convert_from': lambda user: user.get('num_purchases',0) > 1,
     'func': lambda user: user.get('num_purchases',0) > 2, 'show_p': True },
    {'name': 'Z01B %CONV% Made 4 Payments', 'convert_from': lambda user: user.get('num_purchases',0) > 2,
     'func': lambda user: user.get('num_purchases',0) > 3, 'show_p': True },
...
#            fork_base = Query(query, 'base')
        ...
        for ui_name, names in [['Armor Games', ['ag']], ['Kongregate', ['kg']], ['Battlehouse', ['bh']], ['Mattermost', ['mm']], ['Facebook', ['fb','MISSING']]]:
