...
weekly_score_pattern = re.compile('^(score|quarry_resources|hive_kill_points|damage_inflicted_pve|trainee_completions|trophies|tokens_looted)_.*_wk([0-9]+)$')
season_score_pattern = re.compile('^(score|quarry_resources|hive_kill_points|damage_inflicted_pve|trainee_completions|trophies|tokens_looted)_.*_s([0-9]+)$')
...
def pruned_stale_entry(entry):
    ...
    for FIELD in ('ui_name', 'real_name', 'social_id', 'kg_avatar_url',
                  'facebook_id', 'kg_id', 'ag_id', 'ag_avatar_url', 'bh_id', 'mm_id',
                  ):
        if FIELD in entry:
