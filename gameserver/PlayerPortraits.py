...
class PlayerPortraits(object):
    def __init__(self, db_client, async_http_map, log_exception_func):
        ...
        if pcache_info is not None:
            ...
            if target_platform:
                ...
                if ret:
                    ...
                    def on_error(f, self):
                        self.log_exception_func('PlayerPortrait update error: '+f.getTraceback().strip())
                        return (False, 'image/png', unknown_person_portrait_50x50_png)
                    ret.addErrback(on_error, self)
                    return ret
