...
class PlayerPortraits(object):
    ...
    def update_complete(self, d, url, frame_platform, time_now, user_id, body = '', headers = {}, status = '500', ui_reason = None):
        ...
        try:
            ...
            # get expiration time
            # note: if expires remains None, the entry will NEVER expire!
            expires = None
            if 'cache-control' in headers:
                match = max_age_regexp.search(headers['cache-control'][-1])
                if match:
                    max_age = int(match.groups()[0])
                    expires = max_age + time_now

            if (expires is None) and ('expires' in headers):
                expires = SpinHTTP.parse_http_time(headers['expires'][-1])

            max_expires = time_now + 30*86400 # never let things expire more than this far into the future
            if (expires is None) or (expires > max_expires): # fallback
                expires = max_expires
